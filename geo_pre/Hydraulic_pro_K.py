...
for m in range(rn.Net.nb):
    for n in range(rn.Net.Bra[m].ns+1):
        ...
        K = torch.tensor([ rn.Net.Bra[m].Sec[n].Lay[i].k for i in range(Net.Bra[0].Sec[0].npas) ], dtype=torch.float32)
...
optimizers = [optim.Adam(model.parameters(), lr=0.005) for model in models]
optimizers_LB = [torch.optim.LBFGS(model.parameters(), lr=0.005,
                                  max_iter=10000,
                                  max_eval=None,
                                  tolerance_grad=1e-8,
...
# 检查是否已经存在训练好的模型
if not os.path.exists('models_K'):
    os.makedirs('models_K')

for i in range(26):
    model_path = f'models_K/model_{i}.pth'
    ...
    if os.path.exists(model_path):
        ...
        def closure():
            ...
            loss = nn.MSELoss()(outputs, Ks[i])
            losses.append(loss.item())
            loss.backward()
            return loss

        # 训练模型
        for epoch in range(80000):  # 迭代次数可以自己调整
            optimizers[i].zero_grad()
            outputs = models[i](Zs[i])
            loss = nn.MSELoss()(outputs, Ks[i])  # 我们使用均方误差作为损失函数
            loss.backward()
            optimizers[i].step()
            # 打印每个epoch的损失
        ...
        # 使用模型预测
        Z_test = Zs[i]
        K_pred = models[i](Z_test)

        # 绘制真实值和预测值
        plt.figure()
        plt.plot(Zs[i].detach().numpy(), Ks[i].detach().numpy(), 'g', label='True value')
        plt.plot(Z_test.detach().numpy(), K_pred.detach().numpy(), 'r--', label='Predicted value')
        plt.xlabel('Z')
        plt.ylabel('K')
        plt.legend()
        plt.savefig(f'Figs/HP/Z_K_{i}.pdf', dpi=300)
        ...
        plt.savefig(f'Figs/HP/Z_K_loss_{i}.pdf', dpi=300)
