...
class MPC:
    ...
    def solve_ocp(self, x0, x_ref, iter, iterations=5000, tol=1e-8):
        """
    ...
    def optimization_step_gradient(self, x0, x_ref):
        ...
        mask = tf.concat([
            tf.zeros_like(self.u[:, :2]),
            tf.ones_like(self.u[:, 2:])
        ], axis=1)
    ...
    def sim(self, x0,  X_ref, T_ref, U_dis=None):
        ...
        for i, t in enumerate(T_ref[:-self.H]):
            ...
            x_true_p = self.sim_plant_system(X_mpc_p[i], u_k, self.t_sample) # 模拟实际系统 放大100倍的


            print('X_mpc',X_mpc[i])
            x_true = self.sim_nn(X_mpc[i], u_k, self.t_sample)*100
            print('x_true',x_true)
            ...
            err_pct = float(abs((x_true_p[4] - X_ref[i]) / abs(X_ref[i])) * 100)
            ...
            log_str = log_str + f'\目标处水位: {x_true_p[4]:.3f}, 误差百分比: {err_pct:.2f}%, IAQ累计: {self.iaq_accum:.2f}'

            log_str = log_str + f'\tOCP-solving-time: {ocp_solving_time:.2e} s'
            log_str = log_str + f'\tcompute_time: {compute_time:.2e} s'
            logging.info(log_str)

        return X_mpc, U_mpc, X_pre, X_mpc_p
