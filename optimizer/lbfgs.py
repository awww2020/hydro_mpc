...
def function_factory(model, loss_fcn, x, y, callback_fcn, epochs, x_test=None, y_test=None,
    ...
    def f(weights):
        ...
        f.iter += 1
        callback_fcn(f.iter, loss_value, epochs, x_test, y_test,
        ...
        if hasattr(callback_fcn, '__self__') and hasattr(callback_fcn.__self__, 'early_stop_triggered_u'):
            if callback_fcn.__self__.early_stop_triggered_u:
                raise StopIteration("Early stopping triggered by epoch_callback_u")
