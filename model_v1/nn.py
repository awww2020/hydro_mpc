...
class NN(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, layers: list, lb: np.ndarray, ub: np.ndarray) -> None:
        ...
        lb = tf.constant(lb, dtype=self.dtype)
        ub = tf.constant(ub, dtype=self.dtype)
        ...
        self.train_loss_results_phys = {}
        self.train_loss_results_phys_norm = {}
        self.train_loss_results_total = {}
    ...
    def loss(self, y_z, y_u, y_z_pred, y_u_pred):
        """Data loss only"""
        y_z0 = y_z[:1, :]
        z0 = y_z_pred[:1, :]
        ...
        y_z_sel = tf.concat([y_z[1:, 0:1], y_z[1:, 2:3],  y_z[1:, -1:]], axis=1)
        y_z_pred_sel = tf.concat([y_z_pred[1:, 0:1], y_z_pred[1:, 2:3], y_z_pred[1:, -1:]], axis=1)

        y_u_sel = tf.concat([y_u[1:, 0:1], y_u[1:, 2:3],  y_u[1:, -1:]], axis=1)
        y_u_pred_sel = tf.concat([y_u_pred[1:, 0:1], y_u_pred[1:, 2:3], y_u_pred[1:, -1:]], axis=1)
        '''
        loss_z = self.loss_object(y_z_sel, y_z_pred_sel)
        loss_u = self.loss_object(y_u_sel, y_u_pred_sel)
        '''
        loss_z = self.loss_object(y_z_sel, y_z_pred_sel) + self.loss_object(y_z0, z0)
        loss_u = self.loss_object(y_u_sel, y_u_pred_sel) + self.loss_object(y_u0, u0)
    ...
    def train_step(self, x, y_z, y_u):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss_z, loss_u = self.loss(y_z, y_u, y_z_pred, y_u_pred)
            loss_total = loss_z + loss_u

        # gradients = tape.gradient(loss, self.model_u.trainable_variables)
        gradients_z = tape.gradient(loss_total, self.model_z.trainable_variables)
        gradients_u = tape.gradient(loss_total, self.model_u.trainable_variables)
        ...
        return loss_z, loss_u, tf.constant(0.0, dtype=self.dtype), tf.constant(0.0, dtype=self.dtype), loss_total
    ...
    def train_adam(self, x, y_z, y_u, epochs=2000, x_test=None, y_z_test=None, y_u_test=None, learning_rate=0.1, val_freq=1000, log_freq=1000,
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.epoch_callback_adam(epoch, epoch_loss_z.result(), epoch_loss_u.result(), epoch_loss_phys.result(),
                                     epoch_loss_phys_norm.result(), epoch_loss_total.result(), epochs, x_test, y_z_test, y_u_test, val_freq, log_freq,
                                verbose)
    ...
    def plot_train_results(self, basename=None):
        ...
        ax.set_ylabel('Loss')
        ax.set_yscale('log')  # Use logarithmic scale for better visualization of large differences

        # Plotting Losses for z, u, phys_raw, and total
        ax.plot(self.train_loss_results_z.keys(), self.train_loss_results_z.values(), label='Lz', linestyle='-',
                color='b')
        ax.plot(self.train_loss_results_u.keys(), self.train_loss_results_u.values(), label='Lu', linestyle='--',
                color='g')
        ax.plot(self.train_loss_results_phys.keys(), self.train_loss_results_phys.values(), label='Lf',
                linestyle='-.', color='r')
        ax.plot(self.train_loss_results_total.keys(), self.train_loss_results_total.values(), label='Lt',
                linestyle=':', color='purple')
        '''
        # Optional: Plotting Accuracy for z if available
        if self.train_accuracy_results_z:
            ax.set_ylabel("Loss / Accuracy")
            ax.plot(self.train_accuracy_results_z.keys(), self.train_accuracy_results_z.values(), label='Accuracy (z)',
                    linestyle='-', color='orange')
            
        if self.train_accuracy_results_u:
            ax.set_ylabel("Loss / Accuracy")
            ax.plot(self.train_accuracy_results_u.keys(), self.train_accuracy_results_u.values(), label='Accuracy (u)',
                    linestyle='-', color='orange')
        '''
        ...
        # Save the figure
        save_fig(fig, f'{self.name}_train_metrics')  # Save the figure without showing it
        plt.close(fig)
        fig.tight_layout()
    ...
    def epoch_callback_adam(self, epoch, epoch_loss_z, epoch_loss_u, epoch_loss_phys, epoch_loss_phys_norm, epoch_loss_total, epochs, x_val=None, y_z_val=None, y_u_val=None, val_freq=1000, log_freq=1000,
                       verbose=1):
        """
        ...
        self.train_loss_results_phys[epoch] = epoch_loss_phys
        self.train_loss_results_phys_norm[epoch] = epoch_loss_phys_norm
        self.train_loss_results_total[epoch] = epoch_loss_total
        ...
        if epoch % val_freq == 0 or epoch == 1:
            ...
            log_str = (
                ...
                f'{float(epoch_loss_z):.4e}, {float(epoch_loss_u):.4e}, '
                f'{float(epoch_loss_phys):.4e}, {float(epoch_loss_phys_norm):.4e}, {float(epoch_loss_total):.4e}'
            )
